...
public class QosSliceCommand extends AbstractShellCommand {
    ...
    private static final String INSERT_QUEUE_ARGS =
            COLOR_ERROR + "Missing the " + BOLD + "Queue config parameters." + RESET +
                    COLOR_ERROR + " Specifying M_DELAY, T_DELAY and C_DELAY is required." +
                    RESET;

    private static final String WRONG_QOS_PARAMS =
            COLOR_ERROR + "Wrong " + BOLD + "Queue config parameters." + RESET +
                    COLOR_ERROR + " Specifying M_DELAY, T_DELAY and C_DELAY is required." +
                    RESET;
    ...
    @Argument(index = 0, name = "command", description = "Command name (|add-meter|add-band|add-qos|clean|create|list|delete|rem-meter|" +
     ...
     * @param sliceName  the name of the Slice
     */
    protected void addQos(String sliceName, String args) {
        ...
        try {
            List<Long> queueArgs = Arrays.stream(args.split(" ").clone()).mapToLong(Long::parseLong).boxed().collect(Collectors.toList());
            if (queueArgs.size() != 3) {
                print(WRONG_QOS_PARAMS);
                return;
            }
            qoSData.addQueue(new QueueConfig(queueArgs.get(0), queueArgs.get(1), queueArgs.get(2)));
        } catch (Exception e) {
            print(WRONG_QOS_PARAMS);
            return;
        }
