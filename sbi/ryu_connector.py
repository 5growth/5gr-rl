...
class Connector(wim_connector.WimConnector):
    def __init__(self, parameters):
        super().__init__()
        self.description = "Specific Ryu Connector developed by MONET team"
        self.logger = logging.getLogger('cttc_rl.sbi.ryu_connector')
    ...
    # in order to save time, used translation method done by MONET team
    @staticmethod
    def translate_call_mtp_cop(call):
        trans_call = {"contextId": call["contextId"], "aEnd": call["aEnd"], "trafficParams": call["trafficParams"],
                      "callId": call["callId"], "transportLayer": {"direction": call["transportLayer"]["direction"],
                                                                   "layer": call["transportLayer"]["layer"],
                                                                   "layerId": call["transportLayer"]["layer"]},
                      "zEnd": call["zEnd"], "match": call["match"], "connections": []}
        includePath = call["match"]["includePath"]
        connection_dict = {"contextId": trans_call["contextId"], "connectionId": trans_call["callId"],
                           "zEnd": trans_call["zEnd"], "aEnd": trans_call["aEnd"],
                           "path": trans_call["match"]["includePath"], "transportLayer": trans_call["transportLayer"],
                           "match": call["match"]}
        trans_call["connections"].append(connection_dict)
        return trans_call

    @staticmethod
    def translate_call_cop_mtp(call):
        # translate to MTP format
        trans_call = {"callId": call["callId"],
                      "context": call["contextId"],
                      "aEnd": {"nodeId": call["aEnd"]["nodeId"],
                               "edgeEndId": call["aEnd"]["edgeEndId"],
                               "endpointId": call["aEnd"]["endpointId"]},
                      "zEnd": {"nodeId": call["zEnd"]["nodeId"],
                               "edgeEndId": call["zEnd"]["edgeEndId"],
                               "endpointId": call["zEnd"]["endpointId"]},
                      "transportLayer": {"layer": call["transportLayer"]["layer"],
                                         "direction": call["transportLayer"]["direction"]},
                      "trafficParams": {"reservedBandwitdth": call["trafficParams"]["reservedBandwidth"]}, "match": {}}

        for key in call["connections"][0]["match"].keys():
            if not key == "includePath":
                trans_call["match"][key] = call["connections"][0]["match"][key]
        return trans_call
    ...
    def get_topology(self):
        ...
        """
        # TODO verify it with a real server
        url = self.get_access()
        try:
    ...
    def create_call(self, callId, internal_path, inter_wan_path, edge_paths, src_ip, dst_ip, metadata_call, vlan, bw):
        ...
        try:
            ...
            trans_arp_forward_call_body = Connector.translate_call_mtp_cop(arp_forward_call_body)
            response = requests.post(url_arp_forward, data=json.dumps(trans_arp_forward_call_body),
            ...
            # ARP call backward
            # TODO verify the translation
            trans_arp_backward_call_body = Connector.translate_call_mtp_cop(arp_backward_call_body)
            response = requests.post(url_arp_backward,
                                     data=json.dumps(trans_arp_backward_call_body),
                                     headers=self.headers)
            # logger.info(response.status_code, response.content)
        ...
        try:
            ...
            # IP call forward
            # TODO verify the translation
            trans_ip_forward_call_body = Connector.translate_call_mtp_cop(ip_forward_call_body)
            response = requests.post(url_ip_forward, data=json.dumps(trans_ip_forward_call_body), headers=self.headers)
            # logger.info(response.status_code, response.content)
            ...
            # IP call backward
            # TODO verify the translation
            trans_ip_backward_call_body = Connector.translate_call_mtp_cop(ip_backward_call_body)
            response = requests.post(url_ip_backward, data=json.dumps(trans_ip_backward_call_body),
                                     headers=self.headers)
            # logger.info(response.status_code, response.content)
