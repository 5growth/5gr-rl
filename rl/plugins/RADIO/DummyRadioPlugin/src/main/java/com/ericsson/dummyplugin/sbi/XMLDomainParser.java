...
public class XMLDomainParser {
    ...
    public XMLDomainParser(String xmlFile) {
        ...
        //Parse Radio Coverage Area List
        Element xmlcalist = rootNode.getChild("CoverageAreaList");
        List<Element> xmlca = xmlcalist.getChildren("CoverageArea");
        
        for (int i = 0; i < xmlca.size(); i++) {
            Element cael = (Element) xmlca.get(i);
            String caid = cael.getChildText("Id");
            String geoinfo = cael.getChildText("geographicalinfo");
            String minbwstr = cael.getChildText("MinBw");
            BigDecimal minbw = new BigDecimal(minbwstr);
            String maxbwstr = cael.getChildText("MaxBw");
            BigDecimal maxbw = new BigDecimal(maxbwstr);
            String delaystr = cael.getChildText("Delay");
            BigDecimal delay = new BigDecimal(delaystr);
            String latitudestr = cael.getChildText("Latitude");
            BigDecimal latitude = new BigDecimal(latitudestr);
            String longitudestr = cael.getChildText("Longitude");
            BigDecimal longitude = new BigDecimal(longitudestr);
            String altitudestr = cael.getChildText("Altitude");
            BigDecimal altitude = new BigDecimal(altitudestr);
            String rangestr = cael.getChildText("Range");
            BigDecimal range = new BigDecimal(rangestr);
            RadioCoverageInfo covarea = new RadioCoverageInfo(caid, geoinfo, minbw, maxbw, delay, latitude, longitude, altitude, range);
            radiocoveragelist.add(covarea);
        }
    ...
    public List<RadioCoverageInfo> getRadiocoveragelist() {
        return radiocoveragelist;
    }

    public void setRadiocoveragelist(List<RadioCoverageInfo> radiocoveragelist) {
        this.radiocoveragelist = radiocoveragelist;
    }
