...
file for the VIM Manager service. It collects all the
necessary info from the other modules in this package. It is assumed that:

* every other module in this package has a 'list_opts' function which
  return a dict where
  * the keys are strings which are the group names
  * the value of each key is a list of config options for that group
* the vim_manager.conf package doesn't have further packages
  with config options
...
def list_opts():
    """Grouped list of all the project-specific configuration options.
...
def _import_modules(module_names):
    ...
    for modname in module_names:
        mod = importlib.import_module(
            "vim_manager.conf." + modname)
        if not hasattr(mod, LIST_OPTS_FUNC_NAME):
            msg = (
                "The module 'vim_manager.conf.%s' "
                "should have a '%s' function which returns the config "
                "options." % (modname, LIST_OPTS_FUNC_NAME))
            raise Exception(msg)
        else:
